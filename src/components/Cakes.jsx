...
export default function Cakes() {
  ...
  useEffect(() => {
    const fetchCakes = async () => {
      try {
        const response = await axios.get('http://localhost:4000/getallcakes');
  ...
  const handleDecrement = async (id) => {
    try {
      ...
      if (response.ok) {
        setCakes((prevCakes) =>
          prevCakes.map((cake) =>
            cake.id === id && cake.quantity > 1 ? { ...cake, quantity: cake.quantity - 1 } : cake
          )
        );
      } else {
  ...
  const addToCart = async (id) => {
    try {
      const selectedCake = cakes.find((cake) => cake.id === id);
      if (selectedCake) {
        ...
        setCartItems([...cartItems, selectedCake]);
      }
    } catch (error) {
  ...
  return (
    ...
    <div className="flex flex-col items-center">
      <h1 className="text-2xl font-bold my-4">Cakes</h1>
      ...
      <div className="flex flex-wrap justify-center gap-6">
        {cakes.map((cake) => (
          <div key={cake.id} className="border p-4 rounded-lg w-60">
            <img src={cake.image} alt={cake.name} className="w-full h-56 object-cover rounded-md" />
            <h2 className="text-xl font-semibold mt-2">{cake.name}</h2>
            <p className="text-gray-700">{cake.description}</p>
            <p className="mt-2">Price: {cake.price}</p>
            {/* <p className="mt-2">Quantity: {cake.quantity}</p> */}
            {/* <div className="flex justify-between items-center mt-4">
              <button
                onClick={() => handleDecrement(cake.id)}
                className="bg-gray-300 px-2 rounded-l"
              >
                -
              </button>
              <span className="px-4">{cake.quantity}</span>
              <button
                onClick={() => handleIncrement(cake.id)}
